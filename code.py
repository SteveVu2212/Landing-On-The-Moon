...
#                 weight_update = None
                ...
                weight_update = self.step_size * m_hat / (self.epsilon + np.sqrt(v_hat))
                
                weights[i][param] += weight_update
...
#     max_preference = None
    ...
    max_preferences = np.max(action_values, axis = 1)/tau
    
    
    # Reshape max_preference array which has shape [Batch,] to [Batch, 1]. This allows NumPy broadcasting 
    # when subtracting the maximum preference from the preference of each action.
    reshaped_max_preference = max_preferences.reshape((-1, 1))
...
class Agent(BaseAgent):
    ...
    def agent_step(self, reward, state):
        ...
        experience = self.replay_buffer.append(self.last_state, self.last_action, reward, terminal, state)
        ...
        if self.replay_buffer.size() > self.replay_buffer.minibatch_size:
            ...
            for _ in range(self.num_replay):
                ...
                weights = optimize_network(experiences, self.discount, self.optimizer, self.network, current_q, self.tau)
